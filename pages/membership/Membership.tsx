...
const Membership: React.FC = () => {
  ...
  const tiers = [
    ...
    {
      name: t("tiers.institutionalPartner"),
  ...
  return (
    <div className="pt-20 px-6 max-w-7xl mx-auto pb-40 relative">
      ...
      <div className="text-center mb-32 relative z-10">
        ...
        <h1 className="text-6xl md:text-8xl font-serif-vintage italic mb-6">
          {t("header.title")}{" "}
          <span className="text-amber-600">
            {t("header.titleHighlight") || t("header.title")}
          </span>
        </h1>
        ...
        {tiers.map((tier, i) => (
          ...
          >
            <div className="xl:w-1/3 space-y-8 flex flex-col items-center xl:items-start text-center xl:text-left">
              ...
              <div>
                <h3 className="text-4xl font-heading mb-4 text-white group-hover:text-amber-500 transition-colors leading-tight">
                  {tier.name}
                </h3>
            ...
            <div className="xl:w-2/3 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
              <div>
                <h4 className="text-[11px] uppercase tracking-[0.3em] font-bold text-amber-500 mb-6 flex items-center gap-2">
                  ...
                  {t("sections.privileges")}
                </h4>
                <ul className="space-y-4">
                  {tier.benefits.map((benefit, idx) => (
                    <li
                      key={idx}
                      className="flex items-start gap-3 text-xs font-medium tracking-wide text-gray-300 leading-snug"
                    >
                      <Check
                        size={14}
              ...
              <div>
                ...
                <ul className="space-y-4">
                  {tier.duties.map((duty, idx) => (
                    <li
                      key={idx}
                      className="flex items-start gap-3 text-xs font-medium tracking-wide text-gray-400 italic leading-snug"
                    >
                      <div className="w-1.5 h-1.5 bg-amber-900/60 rounded-full mt-2 shrink-0 border border-amber-600/30" />{" "}
                      {duty}
                    </li>
                  ))}
                </ul>
              ...
              <div className="md:col-span-2 lg:col-span-1">
                ...
                <ul className="space-y-4 mb-8">
                  {tier.criteria.map((item, idx) => (
                    <li
                      key={idx}
                      className="flex items-start gap-3 text-[10px] uppercase font-bold tracking-widest text-gray-500 leading-tight"
                    >
                      <span className="text-amber-900 font-serif-vintage italic text-base -mt-1">
                        {idx + 1}.
                      </span>{" "}
                      {item}
                    </li>
