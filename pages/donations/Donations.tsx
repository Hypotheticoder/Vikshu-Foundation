...
const Donations: React.FC = () => {
  ...
  const donationTypes = [
    {
      id: "general",
      name: t("donationTypes.general.name"),
      icon: <Heart size={18} />,
      desc: t("donationTypes.general.desc"),
    },
    {
      id: "corpus",
      name: t("donationTypes.corpus.name"),
      icon: <Coins size={18} />,
      desc: t("donationTypes.corpus.desc"),
    },
    {
      id: "anonymous",
      name: t("donationTypes.anonymous.name"),
      icon: <ShieldOff size={18} />,
      desc: t("donationTypes.anonymous.desc"),
    },
  ];
  ...
  const handleSelectType = (id: "general" | "corpus" | "anonymous") => {
    ...
    if (typeof window !== "undefined" && window.innerWidth < 1024) {
      ...
      setTimeout(() => {
        ...
        const headerOffset = navEl
          ? Math.round(navEl.getBoundingClientRect().height) + 96
          : 22; // dynamic header height fallback
  ...
  const membershipTiers = [
    {
      name: t("tiers.coreFellowship.name"),
      icon: <Star className="text-amber-500" />,
      desc: "The guardians of the foundation's innermost lineage. A lifetime appointment for the dedicated.",
      voting: t("tiers.coreFellowship.voting"),
      benefits: [
        "Permanent Grand Council Seat",
        "Sanctum of Silence Access",
        "Legacy Naming Rights",
        "Global Delegacy Funding",
      ],
      duties: [
        "Mentorship of Rising Seekers",
        "Strategic Oversight of a Pillar",
        "Endowment Stewardship",
      ],
      criteria: [
        "15+ years of cultural service",
        "Mastery of Sanskrit/Victorian disciplines",
        "Council Unanimity",
      ],
      color: "border-amber-500/50 bg-amber-500/5",
    },
    {
      name: t("tiers.ordinaryMember.name"),
      ...
      voting: t("tiers.ordinaryMember.voting"),
      benefits: [
        "Resolution Voting Power",
        "Priority Retreat Enrollment",
        "Project Leadership Eligibility",
        "Archive Repository Access",
      ],
      duties: [
        "Assembly Meet Attendance",
        "Annual Circle of Action",
        "Local Value Advocacy",
      ],
      criteria: [
        "12-month Associate period",
        "Resonance Assessment Pass",
        "Fellow Recommendation",
      ],
      color: "border-white/10 bg-white/[0.02]",
    },
    {
      name: t("tiers.associateMember.name"),
      ...
      voting: t("tiers.associateMember.voting"),
      benefits: [
        "Public Lecture Access",
        "Digital Wisdom Repository",
        "Induction Pathway",
        "Volunteer Program Entry",
      ],
      duties: [
        "Protocol Adherence",
      ...
      criteria: [
        "Digital Portal Application",
        "Counselor Meet Verification",
  ...
  return (
    <div className="pt-20 px-6 max-w-7xl mx-auto pb-40 relative">
      ...
      <div className="text-center mb-24 relative z-10">
        ...
        <h1 className="text-6xl md:text-[8rem] font-serif-vintage italic mb-6 leading-none">
          {t("header.title")}{" "}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 relative z-10 mb-32">
        ...
        <div className="lg:col-span-4 space-y-4">
          <h3 className="text-[10px] uppercase tracking-[0.4em] font-bold text-gray-500 mb-6 px-4">
            {t("donationTypes.title")}
          </h3>
          {donationTypes.map((type) => (
            ...
            >
              <div
                className={`w-12 h-12 rounded-xl flex items-center justify-center transition-colors ${activeTab === type.id ? "bg-amber-600 text-white" : "bg-white/5 text-amber-500"}`}
              >
                {type.icon}
              </div>
              <div>
                <h4
                  className={`font-heading text-lg mb-2 ${activeTab === type.id ? "text-white" : "text-gray-300"}`}
                >
                  {type.name}
                </h4>
                <p className="text-xs italic leading-relaxed opacity-60 group-hover:opacity-100 transition-opacity">
                  {type.desc}
                </p>
              </div>
              {activeTab === type.id && (
                <motion.div
                  layoutId="activeGlow"
                  className="absolute -inset-1 border border-amber-500/20 rounded-[2.5rem] blur-sm"
                ></motion.div>
              )}
            </button>
          ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          {membershipTiers.map((tier, i) => (
            ...
            >
              ...
              <h3 className="text-2xl font-heading mb-3 group-hover:text-amber-500 transition-colors">
                {tier.name}
              </h3>
              <p className="text-sm text-gray-500 italic mb-8 leading-relaxed line-clamp-2">
                {tier.desc}
              </p>
              ...
              <div className="space-y-8 flex-grow">
                <div>
                  <h4 className="text-[10px] uppercase tracking-[0.3em] font-bold text-amber-600 mb-4 flex items-center gap-2">
                    <Sparkles size={12} /> {t("membership.privileges")}
                  </h4>
                ...
                <div>
                  <h4 className="text-[10px] uppercase tracking-[0.3em] font-bold text-gray-500 mb-4 flex items-center gap-2">
                    <Briefcase size={12} /> {t("membership.duties")}
                  </h4>
                  <ul className="space-y-3">
                    {tier.duties.map((d, idx) => (
                      <li
                        key={idx}
                        className="flex items-start gap-3 text-[11px] text-gray-400 italic"
                      >
                        <div className="w-1 h-1 bg-amber-600/40 rounded-full mt-1.5 shrink-0" />{" "}
                        {d}
                      </li>
                    ))}
                  </ul>
                ...
                <div>
                  <h4 className="text-[10px] uppercase tracking-[0.3em] font-bold text-gray-600 mb-4 flex items-center gap-2">
                    <CheckSquare size={12} /> {t("membership.criteria")}
                  </h4>
                  <ul className="space-y-3">
                    {tier.criteria.map((c, idx) => (
                      <li
                        key={idx}
                        className="flex items-start gap-3 text-[10px] text-gray-500 uppercase tracking-widest font-bold"
                      >
                        <span className="text-amber-900 font-serif-vintage italic text-xs -mt-0.5">
                          {idx + 1}.
                        </span>{" "}
                        {c}
                      </li>
      ...
      <div className="mt-40 grid grid-cols-1 md:grid-cols-2 gap-12 relative z-10">
        ...
        <div className="glass p-12 rounded-[4rem] border border-white/10 flex flex-col justify-center">
          <h4 className="text-[10px] uppercase tracking-[0.5em] font-bold text-amber-500 mb-8">
            {t("directEndowment.title")}
          </h4>
          <div className="space-y-6">
            <div className="pb-6 border-b border-white/5">
              <p className="text-[10px] uppercase tracking-widest text-gray-500 mb-2">
                {t("directEndowment.accountLabel")}
              </p>
              <p className="text-2xl font-heading text-white tracking-widest">
                {t("directEndowment.accountNumber")}
              </p>
            </div>
            <div>
              <p className="text-[10px] uppercase tracking-widest text-gray-500 mb-2">
                {t("directEndowment.routingLabel")}
              </p>
              <p className="text-2xl font-heading text-white tracking-widest">
                {t("directEndowment.routingNumber")}
              </p>
            </div>
          </div>
          <p className="mt-12 text-xs text-gray-500 italic leading-relaxed">
            {t("directEndowment.contactNote")}{" "}
            <span className="text-amber-500">
              {t("directEndowment.contactEmail")}
            </span>
            .
          </p>
        </div>
